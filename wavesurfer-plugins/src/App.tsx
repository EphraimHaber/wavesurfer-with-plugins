...
const ELAN_URL = `${import.meta.env.BASE_URL}sample.eaf`;
...
const XML_VIEWER_THEME_LIGHT = {
  attributeKeyColor: "#2563eb",
  attributeValueColor: "#15803d",
  cdataColor: "#166534",
  commentColor: "#6b7280",
  ...
  lineNumberColor: "#374151",
} as const;

/** High-contrast colors for `react-xml-viewer` on dark page backgrounds */
const XML_VIEWER_THEME_DARK = {
  attributeKeyColor: "#7dd3fc",
  attributeValueColor: "#bbf7d0",
  cdataColor: "#d9f99d",
  commentColor: "#a1a1aa",
  fontFamily: "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace",
  ...
  tagColor: "#fdba74",
  textColor: "#fafafa",
  lineNumberBackground: "#3f3f46",
  lineNumberColor: "#e4e4e7",
} as const;
...
function formatElanTimestamp(sec: number): string {
...
function ElanDemo() {
  ...
  const xmlViewerTheme = prefersDark
  ...
  useEffect(() => {
    ...
    let elanData: ElanParsedData | null = null;
    let elanRegionsApplied = false;

    const applyElanRegions = () => {
      if (elanRegionsApplied || !audioReady || !elanData) return;
      regions.clearRegions();
      for (const ann of elanData.alignableAnnotations) {
        if (ann.start == null) continue;
    ...
    const unsubRegionContent = regions.on(
    ...
    const unsubTable = elan.on("tableViewChange", (model) => {
      setElanTable(model);
    });

    const unsubSelect = elan.on("select", (start) => {
      ws.setTime(start);
    });

    const unsubEafUpdated = elan.on("eafUpdated", () => {
      setXmlModal((m) =>
        m.open ? { open: true, xml: elan.getEafXml() ?? null } : m,
      );
    });
  ...
  const openXmlModal = () => {
    setXmlModal({
      open: true,
      xml: elanPluginRef.current?.getEafXml() ?? null,
    });
  };

  const closeXmlModal = () => setXmlModal({ open: false, xml: null });

  useEffect(() => {
    if (!xmlModal.open) return;
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setXmlModal({ open: false, xml: null });
    };
  ...
  return (
    <div>
      <AudioControls wsRef={wsRef} />
      <p className="record-hint">
        Drag or resize ELAN-backed regions to update <code>TIME_SLOT</code>{" "}
        values in memory. Click region label text to edit the alignable
        transcript; blur to save. The React table updates from the headless
        plugin. Regions created with
        drag-select are not linked to the EAF. View XML shows the live document
        (Escape to close).
      </p>
      <div className="audio-controls">
        <button type="button" onClick={openXmlModal}>
          View EAF XML
        </button>
      </div>
      <div ref={waveformRef} className="wave-container" />
      ...
      <div className="elan-annotations">
        {elanTable ? (
          <table className="wavesurfer-annotations">
            <thead>
              <tr>
                <th className="wavesurfer-time">Time</th>
                {elanTable.columns.map((col) => (
                  <th
                    key={col.tierId}
                    className={`wavesurfer-tier-${col.tierId}`}
            ...
            <tbody>
              {elanTable.rows.map((row) => (
                <tr
                  key={row.alignableId}
                  id={`wavesurfer-alignable-${row.alignableId}`}
                >
                  ...
                  {row.cells.map((cell) => (
                    <td
                      ...
                      className={`wavesurfer-tier-${cell.tierId}`}
                      id={
                        cell.annotationId
                          ? `wavesurfer-annotation-${cell.annotationId}`
                          : undefined
                      }
                      data-ref={row.alignableId}
                      data-start={String(cell.start)}
                      data-end={String(cell.end)}
                      onClick={() => {
                        elanPluginRef.current?.notifyAlignableSelected(
                          row.alignableId,
                        );
                      }}
                    >
      ...
      {xmlModal.open ? (
        ...
        >
          <div
            className="xml-popup-panel"
            role="dialog"
            aria-modal="true"
            aria-labelledby="elan-xml-title"
            onClick={(e) => e.stopPropagation()}
          >
            <header className="xml-popup-head">
              <h2 id="elan-xml-title">ELAN XML</h2>
              <button type="button" onClick={closeXmlModal}>
                Close
              </button>
            </header>
            <div className="xml-popup-body">
              {xmlModal.xml ? (
                <XMLViewer
                  xml={xmlModal.xml}
                  collapsible
                ...
                <p className="xml-popup-empty">No XML loaded yet.</p>
              )}
            </div>
          </div>
        </div>
...
const pluginMeta: Record<PluginId, { title: string; description: string }> = {
  ...
  elan: {
    title: "ELAN",
    description:
      "ELAN table plus regions: drag/resize updates `TIME_SLOT`s in memory; edit labels; inspect XML in a popup (react-xml-viewer).",
  },
};
...
function App() {
  const snippets = useMemo(
    () => ({
      ...
      elan: "import ElanPlugin from './plugins/elan'\n// Headless: elan.on('tableViewChange', setModel), render table in React; elan.on('select', …); get/set/update via getData(), getAnnotationText(id), updateAnnotationText(id, text), syncAlignableTimes(...)",
    }),
    [],
  );
  ...
  return (
    <main className="showcase">
      ...
      <div className="pager">
        ...
        <NavLink to={`/${nextPlugin}`}>
          Next: {pluginMeta[nextPlugin].title}
        </NavLink>
      </div>
